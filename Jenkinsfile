            ...
            steps {
                container ('base') {
                    withCredentials([usernamePassword(passwordVariable : 'DOCKER_PASSWORD' ,usernameVariable : 'DOCKER_USERNAME' ,credentialsId : "$DOCKER_CREDENTIAL_ID" ,)]) {
                        ...
                        sh 'docker build -f Dockerfile -t $REGISTRY/$DOCKERHUB_NAMESPACE/$APP_NAME:$DOCKER_IMAGE_TAG .'
        ...
        stage('build & push chart'){
            ...
            steps {
                container ('base') {
                    withCredentials([usernamePassword(credentialsId: "$GITHUB_CREDENTIAL_ID", passwordVariable: 'GIT_PASSWORD', usernameVariable: 'GIT_USERNAME')]) {
                        script {
                            sh 'helm3 package charts/$APP_NAME --app-version=$DOCKER_IMAGE_TAG --version=$HELM_CHART_VERSION'
    ...
    post { 
        failure { 
            mail(
                ...
                subject: 'rule-manager pipeline is failure', 
                body:'''
                部署编号:${BUILD_NUMBER};
                部署版本:${HELM_CHART_VERSION};
                部署环境:${GITHUB_ORG};
                部署分支:${GIT_BRANCH};
                部署标签:${DOCKER_IMAGE_TAG}''')
        }
        success { 
            mail(
                to: 'lunzhou@yunify.com', 
                cc: 'lunzhou@yunify.com', 
                subject: 'rule-manager pipeline is success', 
                body:'''
                部署编号:${BUILD_NUMBER};
                部署版本:${HELM_CHART_VERSION};
                部署环境:${GITHUB_ORG};
